...
const ToShip = () => {
    ...
    const cancelOrder = async(id)=>{
        setToLoad(id)
        setLoading(true)
        const toCancel = authorizedUser.orders.map((order)=>{
            if(order.orderId === id){
        ...
        <div className="client-to-ship-section">
            {
                authorizedUser.orders != undefined ? 
                    authorizedUser.orders.filter((order)=> order.deliveryStatus === 'toShip')
                    .map((order)=>(
                            ...
                            <div className="right-side">
                                ...
                                <div className={`${loading && toLoad === order.orderId ? 'spinner':''}`} id={order.orderId}>
                                    <div className={`${loading && toLoad === order.orderId ? 'spinner-border':'spinner-custom'}`} role="status">
                                        <button className={`${loading && toLoad === order.orderId ? 'visually-hidden':''}`} type='button' onClick={()=>{cancelOrder(order.orderId)}}>Cancel Orders</button>
                                    </div>
                                </div>
                            </div>
                        </div>
